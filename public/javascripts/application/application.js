...
var Forms = {
  ...
  initForms: function(){
    Forms.validateSignupForm();
    Forms.validateEmailFieldOnBlur();
    Forms.selectInternational();
    Forms.addSpecialCodeButton();
    Forms.codeCheckBoxes();
    Forms.sideBarCartPriceCalculator();
    Forms.checkCartFieldsOnBlur();
  },
    ...
    emailField.on("blur", function() {
      $.ajax({
        ...
        success: function (response, status, xhr) {
          Forms.emailValidationNotice(response, emailField);
        },
  ...
  validatePurchaseOrderForm: function () {
    $("#direct_billings").bind('fieldIsInvalid', function(event, form, el) {
      el.focus();
    }).ketchup();
    $("input[type='submit']", "#direct_billings").on("click", function(e) {
      Forms.disableSubmitButton($(this));
      $("#direct_billings")[0].submit();
    });
  },

  validateSignupForm: function() {
    $("#new_purchase").bind('fieldIsInvalid', function(event, form, el) {
      el.focus();
    }).ketchup();
    $("input[type='submit']", "#new_purchase").on("click", function(e) {
      if (Forms.validateCartFields() == true) {
        if( $('#new_purchase').ketchup("isValid") ) {
          // $("#add_more").remove();
          Gateway.formSubmit();
        }
      } else {
        e.preventDefault();
        alert("You must have at least 1 student and 1 teacher and the value must be numeric.");
      }
    });
  },
  ...
  checkCartFieldsOnBlur: function() {
    var userInputs = $("#add_more input");

    userInputs.on("blur", function() {
      var isValid = Utility.isNumeric($(this).val());;
      if (isValid == false) {
        alert("You must have at least 1 student and 1 teacher and the value must be numeric.");
      } else {
        return true;
  ...
  validateCartFields: function() {
    ...
    $.each(userInputs, function(i,e) {
      isValid = Utility.isNumeric(e);
      if (isValid == false) {
        return false;
      }
    });
  ...
  selectInternational: function() {
    var intCheckbox = $("#international"),
        stateSelect = $("#stripe_params_billing_address_state");
        ...
        $.ajax({
          ...
          success: function(response) {
            if (response == null || response.activated === true) {
              ...
              Utility.calculateDiscount(response);
              codeResponseBlock.append("<p class='green injected'>Discount Applied.</p>");
            }
          },
  ...
  sideBarCartPriceCalculator: function () {
    var userInputs = $("#add_more input");

    userInputs.on("keyup", function () {
      Utility.updateTotals($("#cart_totals").data("cart-discount"));
    });
  }
...
var Gateway = {
    ...
    $("#new_purchase").submit(function(event) {
      var submitButton = $(this).find(".submit-button"),
        ...
        if (directBilling.val() === "") {
          ...
          Forms.disableSubmitButton(submitButton);
        }
    });
  },
  ...
  stripeVerify: function(stripeForm) {
    var self = stripeForm,
        ...
        amount = ($("#total_price").val() * 100); // Stripe expects amount to be in cents
