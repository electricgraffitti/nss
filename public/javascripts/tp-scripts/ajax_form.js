  ...
  $.fn.ajaxSubmit = function (options) { /*jshint scripturl:true */
    ...
    function fileUploadXhr(a) {
      ...
      if (options.extraData) {
        ...
        if (options.extraData.hasOwnProperty(k)) formdata.append(k, options.extraData[k]);
      }
      options.data = null;
        ...
        s.xhr = function () {
          ...
          if (xhr.upload) {
            xhr.upload.onprogress = function (event) {
              var percent = 0;
              if (event.lengthComputable) percent = parseInt((event.position / event.total) * 100, 10);
